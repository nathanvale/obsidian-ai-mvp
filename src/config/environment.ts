...
function createSafeConfigurationError(zodError: z.ZodError): Error {
  ...
  (error as ConfigError).issues = safeIssues;
  (error as ConfigError).safeDebugInfo = createSafeEnvironmentDebugInfo();
    ...
    serializers:
      env.NODE_ENV === 'development'
        ? {
            ...
            config: (configValue: Record<string, unknown>) => {
...
export const secureConfigUtils = {
  ...
  createSecureConfigError(
    ...
    (error as ConfigError).debugContext = {
