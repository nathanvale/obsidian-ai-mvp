...
describe('Environment Security', () => {
  describe('Secret Redaction', () => {
    it('should redact sensitive environment variables', () => {
      ...
      Object.entries(env).forEach(([key, value]) => {
        ...
        if ((key.toLowerCase().includes('vault') && key.toLowerCase().includes('path')) ||
    ...
    it('should have comprehensive redaction keys for ADHD data protection', () => {
      ...
      expect(ENHANCED_REDACT_KEYS.some(key => key.toLowerCase().includes('medical'))).toBe(true);
      expect(ENHANCED_REDACT_KEYS.some(key => key.toLowerCase().includes('health'))).toBe(true);
    });
    ...
    it('should redact personal directory paths', () => {
      ...
      const mockEnv = { OBSIDIAN_VAULT_PATH: testValues[0] };
  ...
  describe('Configuration Security Validation', () => {
    ...
    it('should create secure configuration errors', () => {
      ...
      const error = secureConfigUtils.createSecureConfigError(testMessage, testContext);
      ...
      if (debugContext.safeContext.obsidianVaultPath) {
        expect(debugContext.safeContext.obsidianVaultPath).toMatch(/\[REDACTED/);
      }
